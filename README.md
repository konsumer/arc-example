...
| Get Orders for a customer for a date range                                 | `ordersByCustomer(*customerID, status="OPEN", start=EPOCH, end=NOW)` | `SK={employeeID}, GSI1_SK.between("{status}-{start}", "{status}-{end}")`            |
| Show all Orders in OPEN status for a date range across all customers       | `ordersByStatus(start=EPOCH, end=NOW, status="OPEN")`                | `GSI2_PK=parallell([0...N]), SK.between("{status}#{start}", "{status}#{end}")` |
